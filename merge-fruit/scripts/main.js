...
window.RateLimiter = class {
    ...
    Call() {
        if (-1 === this._timerId) {
            var a = Date.now(), b = a - this._lastCallTime, c = this._interval;
...
window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnCreate(a) {
        const b = a.elementId, c = this.CreateElement(b, a), d = new ElementState(c);
    ...
    _OnSetVisible(a) {
        if (this._autoAttach) {
            var b =
                this._elementMap.get(a.elementId), c = b.GetElement();
    ...
    _OnUpdatePosition(a) {
        if (this._autoAttach) {
            var b = this._elementMap.get(a.elementId), c = b.GetElement();
    ...
    _OnSetCssStyle(a) {
        const b = this.GetElementById(a.elementId), c = a.prop;
        a = a.val;
...
let didCheckWorkerModuleSupport = !1, isWorkerModuleSupported = !1;
...
window.RuntimeInterface = class a {
    constructor(b) {
        ...
        if ("cordova" === this._exportType && this._useWorker) if (isAndroid) {
    ...
    async _Init(b) {
        "macos-wkwebview" === this._exportType ?
            this._SendWrapperMessage({type: "ready"}) : this.IsWebView2Wrapper() && (this._SetupWebView2Polyfills(), this._wrapperComponentIds = (await this._InitWrapper()).registeredComponentIds);
    ...
    async CreateWorker(b, c, d) {
        ...
        if ("cordova" === this._exportType && this._isFileProtocol) return b = await this.CordovaFetchLocalFileAsArrayBuffer(d.isC3MainWorker ? b : this._scriptFolder + b), b = new Blob([b], {type: "application/javascript"}), new Worker(URL.createObjectURL(b), d);
        b = new URL(b, c);
    ...
    async _InitWorker(b, c) {
        ...
        let f = b.workerDependencyScripts || [], g = b.engineScripts;
        f = await Promise.all(f.map(h => this._MaybeGetCordovaScriptURL(h)));
        g = await Promise.all(g.map(h => this._MaybeGetCordovaScriptURL(h)));
        if ("cordova" === this._exportType) for (let h = 0, k = b.projectScripts.length; h < k; ++h) {
            const l = b.projectScripts[h], m = l[0];
    ...
    async _InitDOM(b, c) {
        ...
        for (let [g, h] of f) if (h || (h = g), g === e) try {
    ...
    PostToRuntimeComponentAsync(b,
                                c, d, e, f) {
        const g = nextResponseId++, h = new Promise((k, l) => {
    ...
    _OnMessageFromRuntime(b) {
        ...
        if ("result" === c) this._OnResultFromRuntime(b); else if ("runtime-ready" === c) this._OnRuntimeReady(); else if ("alert-error" === c) this._RemoveLoadingMessage(), alert(b.message); else if ("creating-runtime" ===
            c) this._OnBeforeCreateRuntime(); else throw Error(`unknown message '${c}'`);
    }

    _OnEventFromRuntime(b) {
        const c = b.component, d = b.handler, e = b.data, f = b.responseId;
        if (b = runtimeEventHandlers.get(c)) if (b = b.get(d)) {
            var g = null;
            try {
                g = b(e)
            } catch (h) {
    ...
    _FindRuntimeDOMHandler() {
        for (const b of this._domHandlers) if ("runtime" === b.GetComponentID()) {
    ...
    async _WasmDecodeWebMOpus(b) {
        b =
            await this.PostToRuntimeComponentAsync("runtime", "opus-decode", {arrayBuffer: b}, null, [b]);
    ...
    async _CordovaDoFetchLocalFileAsAsArrayBuffer(b, c, d) {
        try {
            const e = await this.CordovaFetchLocalFile(b), f = await BlobToArrayBuffer(e);
            c(f)
        } catch (e) {
            d(e)
        }
    ...
    _OnWrapperMessage(b) {
        if ("entered-fullscreen" === b) a._SetWrapperIsFullscreenFlag(!0), this._runtimeDomHandler._OnFullscreenChange(); else if ("exited-fullscreen" ===
            b) a._SetWrapperIsFullscreenFlag(!1), this._runtimeDomHandler._OnFullscreenChange(); else if ("object" === typeof b) {
            const c = b.type;
    ...
    _SetupWebView2Polyfills() {
        window.moveTo = (b, c) => {
            this._SendWrapperMessage({type: "set-window-position", windowX: Math.ceil(b), windowY: Math.ceil(c)})
        };
        window.resizeTo = (b, c) => {
            this._SendWrapperMessage({
...
const KEY_CODE_ALIASES = new Map([["OSLeft", "MetaLeft"], ["OSRight", "MetaRight"]]),
    ...
    DISPATCH_SCRIPT_ONLY = {dispatchUserScriptEvent: !0}, DISPATCH_RUNTIME_ONLY = {dispatchRuntimeEvent: !0};
...
async function BlobToSvgImage(a, b, c) {
    ...
    if (e.hasAttribute("width") && e.hasAttribute("height")) {
        const f = e.getAttribute("width"), g = e.getAttribute("height");
...
function KeyboardIsVisible() {
    ...
    if (!a) return !1;
    const b = a.tagName.toLowerCase(), c = new Set("email number password search tel text url".split(" "));
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        window.addEventListener("touchstart", PreventDefaultOnCanvasOrDoc,
            {passive: !1});
        "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", PreventDefaultOnCanvasOrDoc, {passive: !1}), c.addEventListener("pointerdown", PreventDefaultOnCanvasOrDoc)) : c.addEventListener("touchstart", PreventDefaultOnCanvasOrDoc);
        ...
        if (a.IsiOSWebView()) if (window.visualViewport) {
            let d =
                Infinity;
    ...
    _OnInvokeDownload(a) {
        const b = a.url;
        a = a.filename;
        const c = document.createElement("a"), d = document.body;
        c.textContent = a;
        c.href = b;
        c.download = a;
        d.appendChild(c);
    ...
    _PlayPendingMedia() {
        ...
        if (!this._isSilent) for (const b of a) (a = b.play()) && a.catch(c => {
    ...
    _OnScriptCreateWorker(a) {
        const b = a.port2;
        (new Worker(a.url, a.opts)).postMessage({type: "construct-worker-init", port2: b}, [b])
    }
    ...
    _OnExportedToImageSequence(a) {
        window.c3_postToMessagePort({type: "exported-image-sequence", blobArr: a.blobArr, time: a.time, gif: a.gif})
    }
});
"use strict";
self.JobSchedulerDOM = class {
    ...
    async Init() {
        ...
        this._dispatchWorker = await this._runtimeInterface.CreateWorker(a, this._baseUrl, {name: "DispatchWorker"});
        a = new MessageChannel;
        this._inputPort = a.port1;
        this._dispatchWorker.postMessage({type: "_init", "in-port": a.port2}, [a.port2]);
    ...
    async _CreateJobWorker() {
        ...
        var b = this._runtimeInterface._GetWorkerURL(this._GetWorkerScriptFolder() +
            "jobworker.js");
        b = await this._runtimeInterface.CreateWorker(b, this._baseUrl, {name: "JobWorker" + a});
        const c = new MessageChannel, d = new MessageChannel;
        this._dispatchWorker.postMessage({type: "_addJobWorker", port: c.port1}, [c.port1]);
        b.postMessage({type: "init", number: a, "dispatch-port": c.port2, "output-port": d.port2}, [c.port2, d.port2]);
...
self.AudioDOMHandler = class extends self.DOMHandler {
    constructor(a) {
        ...
        this.AddRuntimeMessageHandlers([["create-audio-context", b => this._CreateAudioContext(b)], ["play", b => this._Play(b)], ["stop", b => this._Stop(b)], ["stop-all", () => this._StopAll()], ["set-paused", b => this._SetPaused(b)], ["set-volume", b => this._SetVolume(b)], ["fade-volume", b => this._FadeVolume(b)], ["set-master-volume", b => this._SetMasterVolume(b)],
    ...
    async _CreateAudioContext(a) {
        ...
        if (this._iRuntime.IsExportingToVideo()) this._playMusicAsSound = !0, this._audioContext = new OfflineAudioContext({
            ...
            if ("undefined" !== typeof AudioContext) this._audioContext = new AudioContext(b); else if ("undefined" !== typeof webkitAudioContext) this._audioContext = new webkitAudioContext(b); else throw Error("Web Audio API not supported");
    ...
    _UnblockAudioContext() {
        if (!this._hasUnblocked) {
            ...
            var b = a.createBuffer(1, 220, 22050), c = a.createBufferSource();
            c.buffer = b;
    ...
    _MatchTagLists(a, b) {
        for (const c of b) {
            b = !1;
            for (const d of a) if (self.AudioDOMHandler.EqualsNoCase(d, c)) {
                b = !0;
                break
            }
            if (!b) return !1
    ...
    * audioInstancesMatchingTags(a) {
        if (0 < a.length) for (const b of this._audioInstances) this._MatchTagLists(b.GetTags(), a) && (yield b); else this._lastAudioInstance && !this._lastAudioInstance.HasEnded() && (yield this._lastAudioInstance)
    }
    ...
    * audioInstancesByEffectTag(a) {
    ...
    async _GetAudioInstance(a, b, c, d, e) {
        for (const f of this._audioInstances) if (f.GetUrl() === b && (f.CanBeRecycled() || e)) return f.SetTags(d), f;
    ...
    _MaybeStartTicking() {
        if (0 < this._analysers.size) this._StartTicking(); else for (const a of this._audioInstances) if (a.IsActive()) {
            this._StartTicking();
            break
        }
    }
    ...
    async _Play(a) {
        const b = a.originalUrl, c = a.url, d = a.type, e = a.isMusic, f = a.tags, g = a.isLooping, h = a.vol,
            k = a.pos, l = a.panning, m = a.stereoPan;
        let n = a.off;
    ...
    async _FadeVolume(a) {
        const b = a.tags, c = a.vol, d = a.duration;
        a = a.stopOnEnd;
    ...
    async _Preload(a) {
        const b = a.originalUrl, c = a.url, d = a.type;
        a = a.isMusic;
        try {
    ...
    _SetEffectParam(a) {
        var b = a.tags;
        const c = a.index, d = a.param, e = a.value, f = a.ramp;
        a = a.time;
    ...
    async _LoadAudioInstance(a, b) {
        if (3 !== b) {
            var c = a.bufferOriginalUrl, d = a.bufferUrl, e = a.bufferType, f = a.isMusic, g = a.tags, h = a.isLooping,
                k = a.volume, l = a.playbackTime;
            if (!f || 1 !== b) if (f || 2 !== b) {
                b = null;
                try {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
    ...
    async _Fetch() {
        ...
        if ("cordova" === a.GetExportType() && a.IsRelativeURL(this._url) && a.IsFileProtocol()) this._audioData =
            await a.CordovaFetchLocalFileAsArrayBuffer(this._url); else {
...
self.C3AudioInstance = class {
    constructor(a, b, c) {
        ...
        this._stereoPan = 0;
        this._isStopped = !0;
        this._isLooping = this._resumeMe = this._isPaused =
            !1;
        this._volume = 1;
        this._isMuted = !1;
    ...
    SetPanXYZA(a, b, c, d) {
        if (this._isPannerEnabled) {
            var e = this._pannerPosition, f = this._pannerOrientation, g = Math.cos(d);
            d = Math.sin(d);
    ...
    LoadPanState(a) {
        if (a) {
            ...
            var b = this._pannerNode, c = a.pos;
...
let configureOptions = null, startPromise = null;
...
self.C3AdUtils = {
    Success: function (a, b) {
        a(null, b)
    }, Failure, StartAdmobPlus: async function (a, b) {
    ...
    UpdateAdmobPlusRequest, SetInitConfigureOptions: function (a, b, c, d, e, f, g) {
...
"use strict";
const C3AdUtils = self.C3AdUtils, Success$jscomp$1 = C3AdUtils.Success, Failure$jscomp$1 = C3AdUtils.Failure, Events = {
    get LOAD() {
...
class C3BannerAd {
    ...
    Load(a, b) {
        ...
        a = {
            ...
            size: this._size, position: this._position, offset: 0, npa: "NON_PERSONALIZED" === a ? "1" : "0"
        };
...
"use strict";
const C3AdUtils$jscomp$1 = self.C3AdUtils, Success$jscomp$2 = C3AdUtils$jscomp$1.Success,
    Failure$jscomp$2 = C3AdUtils$jscomp$1.Failure, Events$jscomp$1 = {
        get LOAD() {
...
class C3InterstitialAd {
    ...
    Load(a, b) {
        ...
        this._adLoading = !0;
        this._loadSuccess = this._adShowing =
            !1;
        this._loadPromise = null;
        this._loadPromise = this.GetLoadPromise();
...
"use strict";
const C3AdUtils$jscomp$2 = self.C3AdUtils, Success$jscomp$3 = C3AdUtils$jscomp$2.Success,
    Failure$jscomp$3 = C3AdUtils$jscomp$2.Failure, Events$jscomp$2 = {
        get LOAD() {
...
"use strict";
const C3AdUtils$jscomp$3 = self.C3AdUtils, Success$jscomp$4 = C3AdUtils$jscomp$3.Success,
    Failure$jscomp$4 = C3AdUtils$jscomp$3.Failure, Events$jscomp$3 = {
        get LOAD() {
...
"use strict";
const C3AdUtils$jscomp$4 = self.C3AdUtils, Success$jscomp$5 = C3AdUtils$jscomp$4.Success,
    Failure$jscomp$5 = C3AdUtils$jscomp$4.Failure, IOS = self.cordova ? "ios" === self.cordova.platformId : !1,
...
class C3Consent {
    ...
    async GetTrackingAuthorizationStatus() {
        if (ANDROID) return this._trackingStatusString =
            "not-determined";
        const a = self.cordova.plugins.idfa, b = await a.getInfo();
...
const md5cycle = (a, b) => {
    ...
    ii = (a, b, c, d, e, f, g) => cmn(c ^ (b | ~d), a, b, e, f, g), md51 = a => {
        const b = a.length,
...
"use strict";
const C3AdUtils$jscomp$5 = self.C3AdUtils, C3Consent$jscomp$1 = self.C3Consent, C3BannerAd$jscomp$1 = self.C3BannerAd,
    C3InterstitialAd$jscomp$1 = self.C3InterstitialAd, C3RewardedAd$jscomp$1 = self.C3RewardedAd,
    C3RewardedInterstitialAd$jscomp$1 = self.C3RewardedInterstitialAd;
let bannerAd = null, interstitialAd = null, videoAd = null, rewardedAd = null, rewardedInterstitialAd = null;
const api$jscomp$4 = {}, addHandler = (a, b) => {
    api$jscomp$4[a] = b
};
...
"use strict";
const api$jscomp$5 = {}, addHandler$jscomp$1 = (a, b) => api$jscomp$5[a] = b,
    sleep = a => new Promise(b => setTimeout(b, a));
let bannerState = null, intState = null, videoState = null, rewardedState = null, rewardedIntState = null;
...
"use strict";
const C3AdUtils$jscomp$6 = self.C3AdUtils, api$jscomp$6 = {}, addHandler$jscomp$2 = (a, b) => {
    api$jscomp$6[a] = b
};
let init$jscomp$10 = !1, readyPromise = null;
...
addHandler$jscomp$2("ShowRewardedAdvert", async a => {
    await isReady(a) && self.adBreak({
        type: "reward", beforeAd: () => suspendRuntime(), afterAd: () => resumeRuntime(), adBreakDone: b => {
            switch (b.breakStatus) {
                case "viewed":
                case "dismissed":
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _SuspendRuntime() {
        this.GetRuntimeInterface().PostToRuntimeComponent("runtime", "visibilitychange", {hidden: !0})
    }

    _ResumeRuntime() {
        this.GetRuntimeInterface().PostToRuntimeComponent("runtime", "visibilitychange", {hidden: !1})
    }
